...
pub enum Command {
    ...
    /// Install an addon
    Install {
        #[structopt(parse(try_from_str = str_to_flavor), possible_values = &["retail","ptr","beta","classic_wotlk","classic_tbc","classic_era","classic_era_ptr","classic_ptr","classic_beta"])]
...
fn str_to_flavor(s: &str) -> Result<Flavor, &'static str> {
    match s {
        ...
        "classic_wotlk" => Ok(Flavor::ClassicWotlk),
        ...
        _ => Err("valid values are ['retail','ptr','beta','classic_wotlk','classic_tbc','classic_era','classic_era_ptr','classic_ptr','classic_beta']"),
    }
}
